#!/bin/bash
# ============================================================================
# FCVR (VGLR-FC) evaluation -- Granite-MoE / OBQA, ANSWER-ONLY-LOSS runs
# (branch exp4-train-ans).
#
# Evaluates the pretrained-prior FCVR runs trained by
# fcvr-tuning-granite-obqa-ansmask.sh, one per beta in {0.01, 0.1}. For each:
#   - id_calibration  -> ACC / NLL / ECE / MCE          (vs paper Table 7)
#   - ood_detection   -> answer_entropy + inf_log_var   (ILV orientation vs Table 8)
#
# Each run's results go to their own JSON files (tagged ansmask-...) -- nothing
...
#
# Prereq: Stage-1 adapter ./adapters/granite-obqa-ansmask (answer-only, LoRA on
# Q/K/V AND the expert networks), and per beta the FCVR weights in
# ./router_weights/fcvr/fcvr-granite-obqa-ansmask-pretrained-prior-beta<b>/ .
# ============================================================================
