...
class Qwen2MoeSparseMoeBlock(nn.Module):
    def __init__(self, config):
        ...
        # Gating layer, kept directly in this class for weight loading compatibility
        ...
        # Dropout layer for MC Dropout modes
        self.dropout = nn.Dropout(0.1)
        self.router_mode = getattr(config, 'router_mode', 'top_k')
        self.router_temp = getattr(config, 'router_temp', 1.0)

    def forward(self, hidden_states: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Forward pass with integrated multi-mode routing logic.
        """
        # Use instance attributes as default if not provided
        ...
        # Calculate raw logits, which are used by default for analysis
        router_logits = self.gate(flat_hidden_states)

        # --- Multi-mode routing logic integrated directly here ---
        if mode == "top_k":
            routing_weights = F.softmax(router_logits, dim=1, dtype=torch.float)
            routing_weights, selected_experts = torch.topk(routing_weights, self.top_k, dim=-1)

        elif mode == "sample_k":
            scaled_logits = router_logits / temp
            probabilities = torch.softmax(scaled_logits, dim=-1)
            selected_experts = torch.multinomial(probabilities, self.top_k, replacement=False)
            original_probs = F.softmax(router_logits, dim=1, dtype=torch.float)
            routing_weights = torch.gather(original_probs, 1, selected_experts)

        elif mode == "random_k":
            selected_experts = torch.stack([
                torch.randperm(self.num_experts, device=router_logits.device)[:self.top_k]
                for _ in range(batch_size * sequence_length)
            ])
            routing_weights = torch.full((batch_size * sequence_length, self.top_k), fill_value=1.0 / self.top_k, device=router_logits.device, dtype=hidden_states.dtype)

        elif mode == "fixed_k":
            selected_experts = torch.arange(self.top_k, device=router_logits.device).unsqueeze(0).expand(batch_size * sequence_length, -1)
            routing_weights = torch.full((batch_size * sequence_length, self.top_k), fill_value=1.0 / self.top_k, device=router_logits.device, dtype=hidden_states.dtype)
            ...
            stochastic_logits = self.gate(stochastic_hidden_states)
            
            routing_weights = F.softmax(stochastic_logits, dim=1, dtype=torch.float)
            routing_weights, selected_experts = torch.topk(routing_weights, self.top_k, dim=-1)
            
            if mode == "mc_dropout_stochastic_logits":
                router_logits = stochastic_logits

        elif mode == "all_weighted":
            # This mode routes to ALL experts, weighted by the softmax of their logits.
            selected_experts = torch.arange(self.num_experts, device=router_logits.device).unsqueeze(0).expand(batch_size * sequence_length, -1)
            routing_weights = F.softmax(router_logits, dim=1, dtype=torch.float)

        else:
            raise ValueError(f"Invalid routing mode: {mode}")

        # Normalize the weights of the top-k experts if configured
        if self.norm_topk_prob:
            routing_weights /= routing_weights.sum(dim=-1, keepdim=True)
        
        routing_weights = routing_weights.to(hidden_states.dtype)
        
        # --- The rest of the original forward pass logic remains unchanged ---
        ...
        shared_expert_output = F.sigmoid(self.shared_expert_gate(flat_hidden_states)) * shared_expert_output
...
class Qwen2MoeForCausalLM(Qwen2MoePreTrainedModel, GenerationMixin):
    ...
    def change_router_dropout_rate(self, dropout_rate: float = 0.1):
        ...
        for layer in decoder_layers:
            layer.mlp.dropout = nn.Dropout(dropout_rate)
