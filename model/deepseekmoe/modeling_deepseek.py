...
class MoEGate(nn.Module):
    def __init__(self, config):
        ...
        self.dropout = nn.Dropout(0.1) # default 0.1
    ...
    def forward(self, hidden_states, mode="top_k", temp=1.0):
        """
        A refactored forward method with multiple routing strategies.

        Args:
            hidden_states (`torch.Tensor`): Input states.
            mode (`str`): The routing mode to use.
            temp (`float`): Temperature for temperature-based sampling.

        Returns:
            topk_idx (`torch.Tensor`): Indices of the chosen experts.
            topk_weight (`torch.Tensor`): Weights/gates for the chosen experts.
            aux_loss (`torch.Tensor` or `None`): The auxiliary loss for load balancing.
        """
        bsz, seq_len, h = hidden_states.shape
        flat_hidden_states = hidden_states.view(-1, h)
        
        # --- Common part: Calculate logits ---
        logits = F.linear(flat_hidden_states, self.weight, None)
        
        # --- MODIFICATION: Multi-mode routing logic ---
        if mode == "top_k":
            if self.scoring_func == 'softmax':
                scores = logits.softmax(dim=-1)
            else:
                raise NotImplementedError(f'insupportable scoring function: {self.scoring_func}')
            topk_weight, topk_idx = torch.topk(scores, k=self.top_k, dim=-1, sorted=False)
        
        elif mode == "sample_k":
            scaled_logits = logits / temp
            probabilities = torch.softmax(scaled_logits, dim=-1)
            topk_idx = torch.multinomial(probabilities, self.top_k, replacement=False)
            # For sample_k, we still use the original scores for weighting, not the temp-scaled ones
            scores = logits.softmax(dim=-1)
            topk_weight = torch.gather(scores, 1, topk_idx)
            
        elif mode == "random_k":
            topk_idx = torch.stack([
                torch.randperm(self.n_routed_experts, device=logits.device)[:self.top_k]
                for _ in range(bsz * seq_len)
            ])
            topk_weight = torch.full((bsz * seq_len, self.top_k), fill_value=1.0 / self.top_k, device=logits.device, dtype=hidden_states.dtype)

        elif mode == "fixed_k":
            topk_idx = torch.arange(self.top_k, device=logits.device).unsqueeze(0).expand(bsz * seq_len, -1)
            topk_weight = torch.full((bsz * seq_len, self.top_k), fill_value=1.0 / self.top_k, device=logits.device, dtype=hidden_states.dtype)

        elif mode == "all_weighted":
            topk_idx = torch.arange(self.n_routed_experts, device=logits.device).unsqueeze(0).expand(bsz * seq_len, -1)
            # Use the softmax of the original logits as weights
            topk_weight = logits.softmax(dim=-1).type_as(hidden_states)

        elif mode == "mc_dropout_original_logits" or mode == "mc_dropout_stochastic_logits":
            self.dropout.train() 
            stochastic_hidden_states = self.dropout(flat_hidden_states)
            stochastic_logits = F.linear(stochastic_hidden_states, self.weight, None)
            
            if self.scoring_func == 'softmax':
                stochastic_scores = stochastic_logits.softmax(dim=-1)
            else:
                raise NotImplementedError(f'insupportable scoring function: {self.scoring_func}')
                
            topk_weight, topk_idx = torch.topk(stochastic_scores, k=self.top_k, dim=-1, sorted=False)
            
            # The original logits are used for aux loss unless specified otherwise
            if mode == "mc_dropout_stochastic_logits":
                scores = stochastic_scores # Use stochastic scores for aux loss calculation
            else:
                scores = logits.softmax(dim=-1) # Use original scores for aux loss

        else:
            raise ValueError(f"Invalid routing mode: {mode}")


        # Norm gate to sum to 1
...
class DeepseekMoE(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.router_temp = config.router_temp
    ...
    def forward(self, hidden_states):
        ...
        topk_idx, topk_weight, aux_loss = self.gate(hidden_states, mode=self.router_mode, temp=self.router_temp)
...
class DeepseekForCausalLM(DeepseekPreTrainedModel):
    ...
    def change_router_dropout_rate(self, dropout_rate: float = 0.1):
        """
        Change the dropout rate of the model. This will change the dropout rate of all layers in the model.
        """
        moe_model = self.model
        decoder_layers = moe_model.layers
        for layer in decoder_layers[1:]:
            layer.mlp.gate.dropout = nn.Dropout(dropout_rate)


    def change_router_mode(self, mode: str="random_k", temp: float = 1.0):
        """
        Change the router mode of the model. The router mode can be either "train" or "eval".
        """
        moe_model = self.model
        decoder_layers = moe_model.layers
        for layer in decoder_layers[1:]:
            # layer: GraniteMoeDecoderLayer
            layer.mlp.router_mode = mode
            layer.mlp.router_temp = temp

    def change_router_mode_for_layers(self, mode: str = "random_k", layer_indices=None, temp: float = 1.0):
        """
        Change the router mode of specific layers in the model.

        Args:
            mode (str): The router mode to set.
            layer_index_list (list or None): List of layer indices to change. If None, change all layers.
        """
        moe_model = self.model
        decoder_layers = moe_model.layers
        if layer_indices is None:
            print("No layer indices provided, changing all layers' routing mode...")
            layer_indices = range(len(decoder_layers))
        for idx in layer_indices:
            decoder_layers[idx].mlp.router_mode = mode
            decoder_layers[idx].mlp.router_temp = temp
