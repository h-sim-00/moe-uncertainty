...
# Granite-MoE / OBQA. Runs BOTH tasks: ID calibration + OoD detection.
#
...
#
# The weights path is DERIVED from the same knobs the training run used
# (temperature_mode + run_suffix), so eval reads exactly what training wrote:
#   ./router_weights/vtsr_${TEMPERATURE_MODE}/vtsr-granite-obqa-${RUN_SUFFIX}/
#
# Plain bash for a tmux session over ssh (NO SLURM). Runs from the repo root.
#
# Prereq: the VTSR training run (vtsr-tuning-granite-obqa.sh) has completed and
#         its layer_*_weights.pt exist, PLUS the Stage-1 adapter and 32 MAP
#         routers that the reconstruction loads.
# ============================================================================
...
VTSR_WEIGHTS_DIR="./router_weights/vtsr_${TEMPERATURE_MODE}/vtsr-${MODEL_SHORTCODE}-${DATASET_SHORTCODE}-${RUN_SUFFIX}"
...
for l in "${LAYERS[@]}"; do
    if [ ! -f "${VTSR_WEIGHTS_DIR}/layer_${l}_weights.pt" ]; then
        ...
        echo "       Run vtsr-tuning-granite-obqa.sh first." >&2
        exit 1
    fi
done
...
echo "===================================================="
echo "VTSR evaluation: ${MODEL_SHORTCODE} on ${DATASET_SHORTCODE}"
echo "  layers=${LAYERS[*]} mode=${TEMPERATURE_MODE} suffix=${RUN_SUFFIX}"
...
python evaluate_vtsr.py \
    ...
    --seed "$SEED" \
    2>&1 | tee "logs/vtsr-eval-id-calib.log"
...
python evaluate_vtsr.py \
    ...
    --batch_size "$BATCH_SIZE" \
    --seed "$SEED" \
    2>&1 | tee "logs/vtsr-eval-ood-detect.log"

echo "===================================================="
echo "VTSR evaluation complete."
