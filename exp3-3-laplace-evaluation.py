# filename: exp3-laplace-inference.py
import os
import argparse
import wandb
import torch
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--param_num", type=str, choices=["1b", "3b"], required=True, help="Parameter size of the model (1b or 3b).")
    parser.add_argument("--laplace_path", type=str, required=True, help="Path to the saved/fitted Laplace object.")
    parser.add_argument("--adapter_path", type=str, required=True, help="Path to the adapter model directory.")
...
def main():
    ...
    # --- Configure WandB ---
    project_name = "exp3_laplace_inference"
    exp_name = f"eval_{os.path.splitext(os.path.basename(args.laplace_path))[0]}_on_{args.dataset_name}"
    ...
    if not trainable_params:
        raise ValueError("No trainable parameters found in the loaded model.")
    
    with torch.no_grad():
        for i in tqdm(range(0, len(questions), args.batch_size), desc=f"Manual Laplace Inference on {args.dataset_name}"):
            ...
            # --- THE FIX ---
            # 1. Sample all N weight vectors at once.
            # This returns a tensor of shape [n_samples, n_params].
            sampled_param_vectors = la.sample(n_samples=args.num_samples)

            batch_mc_probs = []
            # 2. Loop through the collected samples.
            for single_sampled_vector in sampled_param_vectors:
                # 3. Load one sampled weight vector into the model.
                # `single_sampled_vector` is now guaranteed to have the correct 1D shape.
                vector_to_parameters(single_sampled_vector, trainable_params)
                
                # 4. Perform a standard forward pass with the new weights.
    ...
    nll_avg, nll_std = np.mean(nlls), np.std(nlls)
    
    entropies = (-torch.sum(final_probs_tensor * torch.log(final_probs_tensor + 1e-9), dim=-1)).tolist()
    ent_avg, ent_std = np.mean(entropies), np.std(entropies)
