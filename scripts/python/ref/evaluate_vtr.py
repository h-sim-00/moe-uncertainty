...
from model.routers.archives.vtr import VariationalTemperatureRouter, evaluate_vtr

def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate a fine-tuned Variational Temperature Router.")
    parser.add_argument("--model_shortcode", type=str, default="granite")
    parser.add_argument("--base_adapter_path", type=str, required=True)
    parser.add_argument("--router_weights_dir", type=str, required=True, help="Path to the directory of saved VTR weights.")
    parser.add_argument("--batch_size", type=int, default=8)
    parser.add_argument("--results_csv_path", type=str, default="./results/evaluation_vtr.csv")
...
def main():
    ...
    print(f"Loading trained VTRs from '{args.router_weights_dir}'")
