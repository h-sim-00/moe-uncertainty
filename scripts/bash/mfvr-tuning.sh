#!/bin/bash
#SBATCH --job-name=mfvr_tuning_prog
#SBATCH --output=/vol/bitbucket/al1624/FIP/albus-bayesian-moe-router/logs/slurm/slurm_%j_mfvr_tuning_prog.log
...
cp ./scripts/python/mfvr-tuning.py .
...
for DATASET_SHORTCODE in "${DATASET_SHORTCODES[@]}"; do
    echo "===================================================="
    echo "Starting Progressive MFVR Fine-tuning"
    ...
    for i in $(seq 31 -1 27); do
        ...
        echo "  - Loading Pre-trained MFVR Layers: [${LOAD_LAYERS[@]}]"
        echo "----------------------------------------------------"

        python mfvr-tuning.py \
